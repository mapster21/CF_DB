...
import pandas as pd
import numpy as np
# Unit test for data frame equality
np.random.seed(30)
df_old = pd.DataFrame(np.random.randn(4,5))
df_old
np.random.seed(30)
df_new = pd.DataFrame(np.random.randn(4,5))
df_new
df_old.equals(df_new) #Equality check here, returns boolean expression: True/False
